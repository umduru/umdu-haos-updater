...
def download_update(info: UpdateInfo, orchestrator=None) -> Path:
    ...
    with _download_progress(orchestrator, info.version):
        try:
            ...
            _LOGGER.exception("Не удалось выполнить загрузку бандла")
            raise DownloadError("Failed to download bundle") from e
        except Exception as e:
            _LOGGER.exception("Неожиданная ошибка при загрузке бандла")
            raise DownloadError("Unexpected error during bundle download") from e
