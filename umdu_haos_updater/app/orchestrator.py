...
class UpdateOrchestrator:
    ...
    def set_mqtt_service(self, mqtt_service: MqttService | None) -> None:
        """Задаёт MQTT сервис, корректно останавливая предыдущий."""
        if self._mqtt_service and self._mqtt_service is not mqtt_service:
            try:
                self._mqtt_service.stop()
            except Exception as e:
                _LOGGER.debug("Ошибка остановки предыдущего MQTT-клиента: %s", e)
    ...
    def publish_state(self, installed: str | None = None, latest: str | None = None, in_progress: bool | None = None) -> None:
        ...
        if not self._mqtt_service:
            return
        # Гарантируем, что ошибки из is_ready() не прервут основной цикл
        try:
            if not self._mqtt_service.is_ready():
                return
        except Exception as e:
            _LOGGER.warning("Ошибка проверки готовности MQTT: %s", e)
            return
